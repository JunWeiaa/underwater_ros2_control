...
def _build_spawn_sdf(
    ...
    tree = ET.parse(sdf_template_file)
    root = tree.getroot()
...
def _should_start_gz(raw_value):
    ...
    if _ros_topic_exists("/clock"):
        print("[visualize.launch.py] /clock already exists; skipping Gazebo launch")
        return False

    print("[visualize.launch.py] /clock not found; launching Gazebo")
    return True
...
def _should_start_rviz(raw_value, merge_tf):
    ...
    if not merge_tf:
        print("[visualize.launch.py] private TF mode; launching a namespaced RViz")
        return True

    if _rviz_node_exists():
        print("[visualize.launch.py] RViz node already exists; skipping RViz launch")
        return False

    print("[visualize.launch.py] RViz node not found; launching RViz")
    return True
...
def _should_bridge_clock(raw_value):
    ...
    if _ros_topic_exists("/clock"):
        print("[visualize.launch.py] /clock already exists; skipping clock bridge")
        return False

    print("[visualize.launch.py] /clock not found; enabling clock bridge")
    return True
...
def launch_setup(context, *args, **kwargs):
    ...
    default_sdf_path = os.path.join(
        get_package_share_directory(pkg_description), "worlds", world + ".world"
    )
    ...
    sdf_template_file = os.path.join(pkg_path, "sdf", "model.sdf")
    sdf_xml = _build_spawn_sdf(
        sdf_template_file,
        robot_name,
        robot_namespace,
        controller_config_file,
    ...
    sdf_file = _write_temp_file(f"{robot_name}_", ".sdf", sdf_xml)
...
def generate_launch_description():
    world = DeclareLaunchArgument(
        "world",
        default_value="bluerov2_heavy_underwater",
    ...
    height = DeclareLaunchArgument(
        "height", default_value="-1", description="Init height in simulation"
    )
